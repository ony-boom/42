...
/*   Updated: 2024/04/18 07:22:43 by rony-lov         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
		...
		t_format_config_modifier *modifier, int to_print_len)
{
	...
	set_base_pad(&format->space_pad, modifier, adjusted_print_len, FALSE);
}
...
int	print_hex_max_width(long long n, t_number_format *format, t_bool show_minus)
{
	...
	if (pad.padding_char == (char)ZERO && format->hex_format.prefix_0x)
		printed += print_hex_prefix(n, format->hex_format.use_upper);
...
int	print_hex(unsigned int n, t_bool use_upper, t_format_config *config)
{
	int				n_len;
	int				printed;
	t_number_format	format;
	...
	printed = 0;
	n_len = get_hex_len(n);
	format = build_hex_format(config, n, n_len);
	format.is_unsigned = TRUE;
	format.hex_format.use_upper = use_upper;
	printed += print_number_with_format(n, &format, print_hex_max_width);
	return (printed);
}
